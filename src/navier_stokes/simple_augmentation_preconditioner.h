...
//LIC// 
//LIC//====================================================================
#ifndef OOMPH_SIMPLE_AUGMENTATION_PRECONDITIONERS_HEADER
#define OOMPH_SIMPLE_AUGMENTATION_PRECONDITIONERS_HEADER
...
namespace Simple_Augmentation_Preconditioner_Subsidiary_Operator_Helper
{
...
//=============================================================================
class SimpleAugmentationPreconditioner 
  : public BlockPreconditioner<CRDoubleMatrix>
{
  ...
  SimpleAugmentationPreconditioner():BlockPreconditioner<CRDoubleMatrix>()
  {
    // Null the pointers:
  ...
  /// destructor
  virtual ~SimpleAugmentationPreconditioner()
  {
    this->clean_up_memory();
  }

  /// Broken copy constructor
  SimpleAugmentationPreconditioner (const SimpleAugmentationPreconditioner&)
  {
    BrokenCopy::broken_copy("SimpleAugmentationPreconditioner");
  }

  /// Broken assignment operator
  void operator=(const SimpleAugmentationPreconditioner&)
  {
    BrokenCopy::broken_assign(" SimpleAugmentationPreconditioner");
  }

  /// Setup method for the SimpleAugmentationPreconditioner.
  void setup();
...
//    }
    ...
    else
    {
      ...
      for(unsigned l_i = 0; l_i < N_lagrange_doftypes; l_i++)
      {
        ...
        const unsigned l_ii = N_fluid_blocks + l_i;
    ...
    if(Using_superlu_ns_preconditioner)
    {
      ...
      Vector<unsigned> fluid_block_indices(N_fluid_blocks,0);
      for (unsigned b = 0; b < N_fluid_blocks; b++) 
      {
  ...
  void set_bulk_mesh(Mesh* mesh_pt)
  {
    Bulk_mesh_pt = mesh_pt;
  }

  void set_boundary_mesh(Mesh* mesh_pt)
  {
    Boundary_mesh_pt = mesh_pt;
  }
  ...
  void set_meshes(const Vector<Mesh*> &mesh_pt)
  {
    ...
    if(is_master_block_preconditioner())
    {
      ...
      // Set the meshes
      this->set_mesh(0,mesh_pt[0]);
      this->set_mesh(1,mesh_pt[1]);
    }
  ...
  unsigned N_fluid_blocks;

  /// \short The number of velocity dof types.
  unsigned N_velo_blocks;
...
}; // end of SimpleAugmentationPreconditioner class


//========================================================================
/// Setup function for the SimpleAugmentationPreconditioner.
//========================================================================
void SimpleAugmentationPreconditioner::setup()
{
  First_NS_solve = true;
  // For debugging
...
//  }
  ...
  My_ndof_types_in_mesh.resize(2,0);
  My_ndof_types_in_mesh[0] = 3;
  My_ndof_types_in_mesh[1] = 3;
...
//    std::cout << "n_dof_types: " << n_dof_types << std::endl; 
  ...
  // The number of velocity blocks, 4 in this problems, for u, v, uc, vc
  N_velo_blocks = 4;

  // Fluid has +1 for the pressure.
  N_fluid_blocks = N_velo_blocks + 1;
//    std::cout << "N_fluid_blocks: " << N_fluid_blocks << std::endl; 

  // The rest are Lagrange multiplier DOF types.
  N_lagrange_doftypes = n_dof_types - N_fluid_blocks;
//    std::cout << "N_lagrange_doftypes: " << N_lagrange_doftypes << std::endl; 
  ...
  // Set up the dof to block map
  Vector<unsigned> dof_to_block_map(n_dof_types,0);
  dof_to_block_map[0] = 0;
  dof_to_block_map[1] = 1;
  dof_to_block_map[2] = 4;
  dof_to_block_map[3] = 2;
  dof_to_block_map[4] = 3;
  dof_to_block_map[5] = 5;
...
//    pause("done un replaced blocks"); 
  ...
  // Extract velocity blocks into v_aug_pt
  DenseMatrix<CRDoubleMatrix*> v_aug_pt(N_velo_blocks,N_velo_blocks,0);
  for(unsigned row_i = 0; row_i < N_velo_blocks; row_i++)
  {
    for(unsigned col_i = 0; col_i < N_velo_blocks; col_i++)
    {
...
//  {
    ...
    Vector<CRDoubleMatrix*> mm_pt(2,0);

    // Get the current Lagrange DOF type.
    // Recall that the ordering is:
    // | Fluid DOF types | Lagrange multiplier DOF types
    //   u v w uc vc wc p  L1, ....
    unsigned lgrmult_dof_type = 5;


    // Get blocks (5,2) and (5,3)
    mm_pt[0] = new CRDoubleMatrix;
    this->get_block(5,2,*mm_pt[0]);
    mm_pt[1] = new CRDoubleMatrix;
    this->get_block(5,3,*mm_pt[1]);

    // Create both the w_i and inv_w_i matrices.
    CRDoubleMatrix* w_pt 
      = new CRDoubleMatrix(this->Block_distribution_pt[5]);
...
//    {
      ...
      unsigned long w_nrow_local 
        = this->Block_distribution_pt[5]->nrow_local();

      // The first row, for the column offset (required in parallel).
      unsigned w_first_row 
        = this->Block_distribution_pt[5]->first_row();

      // A vector to contain the results of mass matrices squared.
      Vector<double> w_i_diag_values(w_nrow_local,0);
      Vector<double> invw_i_diag_values(w_nrow_local,0);

      Vector<int> w_i_column_indices(w_nrow_local);
      Vector<int> w_i_row_start(w_nrow_local+1);
...
//        mm_pt[m_i]->multiply(*mm_pt[m_i],*temp_mm_sqrd_pt);
...
//        Vector<double> m_diag = temp_mm_sqrd_pt->diagonal_entries();
        ...
        for(unsigned long row_i = 0; row_i < w_nrow_local; row_i++)
        {
...
//        delete temp_mm_sqrd_pt; temp_mm_sqrd_pt = 0;
      ...
      // Calculate the entries of W and inv W
      for(unsigned long row_i = 0; row_i < w_nrow_local; row_i++)
      {
        invw_i_diag_values[row_i] = Scaling_sigma / w_i_diag_values[row_i];
        w_i_diag_values[row_i] /= Scaling_sigma;

        // Fill in the column indices and row start.
        w_i_column_indices[row_i] = row_i + w_first_row;
        w_i_row_start[row_i] = row_i;
      }

      w_i_row_start[w_nrow_local] = w_nrow_local;

      // Theses are square matrices. So we use the w_nrow_global for the
      // number of columns.
      unsigned long w_nrow_global 
        = this->Block_distribution_pt[5]->nrow();
      w_pt->build(w_nrow_global,
          w_i_diag_values,
          w_i_column_indices,
          w_i_row_start);
      inv_w_pt->build(w_nrow_global,
          invw_i_diag_values,
          w_i_column_indices,
          w_i_row_start);
//    }
    ...
    Vector<unsigned> mm_locations(2,0);
    mm_locations[0] = 2;
    mm_locations[1] = 3;
...
//
//    std::cout << std::endl;
//    std::cout << "temp_block_to_dof_map: " << std::endl;
//    for (unsigned iii = 0; iii < temp_block_to_dof_map.size(); iii++) 
//    {
//      std::cout << temp_block_to_dof_map[iii] << std::endl; 
//    }
//    pause("printed out the temp mapping"); 
    
    
   
   // Now do the replacement of all blocks in v_aug_pt 
   for (unsigned block_row_i = 0; 
        block_row_i < N_velo_blocks; block_row_i++) 
   {
     unsigned temp_dof_row_i = temp_block_to_dof_map[block_row_i];
     for (unsigned block_col_i = 0; 
          block_col_i < N_velo_blocks; block_col_i++) 
     {
...
//
//      DenseMatrix<CRDoubleMatrix* > f_subblock_pt(N_fluid_blocks,
//                                                  N_fluid_blocks,0);
//      // put in v_aug_pt:
//      for(unsigned v_i = 0; v_i < N_fluid_blocks; v_i++)
//      {
//        for(unsigned v_j = 0; v_j < N_fluid_blocks; v_j++)
//        {
...
//          N_fluid_blocks,0);
//      // put in v_aug_pt:
//      for(unsigned v_i = 0; v_i < N_velo_blocks; v_i++)
//      {
//        for(unsigned v_j = 0; v_j < N_velo_blocks; v_j++)
//        {
...
//      for(unsigned col_i = 0; col_i < N_velo_blocks; col_i++)
//      {
//        f_subblock_pt(N_velo_blocks,col_i) = new CRDoubleMatrix;
//        this->get_block(N_velo_blocks,col_i,
//            *f_subblock_pt(N_velo_blocks,col_i));
//      }
//
//      // Fill in the pressure block B^T
//      for(unsigned row_i = 0; row_i < N_velo_blocks; row_i++)
//      {
//        f_subblock_pt(row_i,N_velo_blocks) = new CRDoubleMatrix;
//        this->get_block(row_i,N_velo_blocks,
//            *f_subblock_pt(row_i,N_velo_blocks));
//      }
...
//      pause("bloodz"); 
    ...
    VectorMatrix<BlockSelector> f_aug_blocks(N_fluid_blocks,
                                             N_fluid_blocks);
    for (unsigned block_i = 0; block_i < N_fluid_blocks; block_i++) 
    {
      for (unsigned block_j = 0; block_j < N_fluid_blocks; block_j++) 
      {
...
//      Vector<LinearAlgebraDistribution*> f_dist_pt(N_fluid_blocks,0);
//      for(unsigned f_i = 0; f_i < N_fluid_blocks; f_i++)
//      {
...
//        (f_dist_pt,f_subblock_pt,*f_aug_pt);
      ...
      // v_aug_pt
//      for(unsigned row_i = 0; row_i < N_fluid_blocks; row_i++)
//      {
//        for(unsigned col_i = 0; col_i < N_fluid_blocks; col_i++)
//        {
...
//      for (unsigned row_i = spatial_dim; row_i < N_velo_blocks; row_i++) 
//      {
//        for (unsigned col_i = spatial_dim; col_i < N_velo_blocks; col_i++) 
//        {
//          navier_stokes_block_preconditioner_pt
...
//      for (unsigned i = 0; i < N_fluid_blocks; i++)
//      {
//        for (unsigned j = 0; j < N_fluid_blocks; j++) 
//        {
...
//      }
    ...
    for(unsigned l_i = 0; l_i < N_lagrange_doftypes; l_i++)
    {
      ...
      if(L_block_preconditioner_pt == 0)
      {
        ...
        Lagrange_multiplier_preconditioner_pt[l_i]->setup(w_pt);
      }
      else
      {
        Lagrange_multiplier_preconditioner_is_block_preconditioner = true;
        Vector<unsigned> l_mult_dof_map;
        l_mult_dof_map.push_back(N_fluid_blocks + l_i);
...
//    if(Doc_time)
  ...
  } // end of SimpleAugmentationPreconditioner::setup


  //========================================================================
  /// \short Clears the memory.
  //========================================================================
  void SimpleAugmentationPreconditioner::clean_up_memory()
  {
    ...
    Lagrange_multiplier_preconditioner_pt.resize(0,0);
  } // end of SimpleAugmentationPreconditioner::clean_up_memory
  ...
  /// that one)
  //========================================================================
  void SimpleAugmentationPreconditioner::
...
//      if (L_prec_type != Exact_lsc_block_preconditioner)
    ...
    unsigned p_dof_number = N_velo_blocks;
    ...
    if(do_pressure)
    {
      ...
      // N_velo_blocks = My_nmesh*spatial_dim;
...
// void SimpleAugmentationPreconditioner::assemble_inv_press_and_veloc_mass_matrix_diagonal
}
#endif
